...
const struct attm_desc htpt_att_db[HTS_IDX_NB] =
{
    ...
    [HTS_IDX_SVC]                   =   {ATT_DECL_PRIMARY_SERVICE, PERM(RD, ENABLE), 0},
    ...
    [HTS_IDX_TEMP_MEAS_VAL]         =   {ATT_CHAR_TEMPERATURE_MEAS, PERM(IND, ENABLE), PERM(RI, ENABLE)},
    // Temperature Measurement Characteristic - Client Characteristic Configuration Descriptor
    [HTS_IDX_TEMP_MEAS_IND_CFG]     =   {ATT_DESC_CLIENT_CHAR_CFG, PERM(RD, ENABLE)|PERM(WR, ENABLE)|PERM(WRITE_REQ, ENABLE), 0},
    ...
    [HTS_IDX_TEMP_TYPE_VAL]         =   {ATT_CHAR_TEMPERATURE_TYPE, PERM(RD, ENABLE), PERM(RI, ENABLE)},
    ...
    [HTS_IDX_INTERM_TEMP_VAL]       =   {ATT_CHAR_INTERMED_TEMPERATURE, PERM(NTF, ENABLE), PERM(RI, ENABLE)},
    // Intermediate Measurement Characteristic - Client Characteristic Configuration Descriptor
    [HTS_IDX_INTERM_TEMP_CFG]       =   {ATT_DESC_CLIENT_CHAR_CFG, PERM(RD, ENABLE)|PERM(WR, ENABLE)|PERM(WRITE_REQ, ENABLE), 0},
    ...
    [HTS_IDX_MEAS_INTV_VAL]         =   {ATT_CHAR_MEAS_INTERVAL, PERM(RD, ENABLE)|PERM(WR, ENABLE)|PERM(WRITE_REQ, ENABLE), PERM(RI, ENABLE) | HTPT_MEAS_INTV_MAX_LEN},
    // Measurement Interval Characteristic - Client Characteristic Configuration Descriptor
    [HTS_IDX_MEAS_INTV_CFG]         =   {ATT_DESC_CLIENT_CHAR_CFG, PERM(RD, ENABLE)|PERM(WR, ENABLE)|PERM(WRITE_REQ, ENABLE), 0},
    // Measurement Interval Characteristic - Valid Range Descriptor
    [HTS_IDX_MEAS_INTV_VAL_RANGE]   =   {ATT_DESC_VALID_RANGE, PERM(RD, ENABLE), PERM(RI, ENABLE)},
};
                            ...
                            uint8_t sec_lvl, struct htpt_db_cfg* params)
{
    ...
    if( status == ATT_ERR_NO_ERROR )
    {
        ...
        memset(htpt_env, 0, sizeof(struct htpt_env_tag));
        ...
        {
            att_perm_type perm = PERM(RD, ENABLE);
            ...
            attmdb_att_set_permission(HTPT_HANDLE(HTS_IDX_MEAS_INTV_VAL), perm);
        }
...
static void htpt_cleanup(struct prf_task_env* env, uint8_t conidx, uint8_t reason)
{
    ...
    struct htpt_env_tag* htpt_env = (struct htpt_env_tag*) env->env;
    htpt_env->ntf_ind_cfg[conidx] = 0;
    ke_state_set(KE_BUILD_ID(env->task, conidx), HTPT_IDLE);
}
