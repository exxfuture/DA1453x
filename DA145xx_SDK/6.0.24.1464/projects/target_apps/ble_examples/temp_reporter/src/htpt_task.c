                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    if (state == HTPT_IDLE)
    {
        ...
        else
        {
            ...
            memset(htpt_env->operation, 0, sizeof(struct htpt_op) + HTPT_TEMP_MEAS_MAX_LEN);
    ...
    if (state == HTPT_IDLE)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                memset(htpt_env->operation, 0, sizeof(struct htpt_op) + HTPT_MEAS_INTV_MAX_LEN);
        ...
        ke_task_id_t const src_id)
{
    ...
    // arch_printf("DEBUG: CUSTOM htpt_task.c gattc_att_info_req_ind_handler called: handle=0x%04X, att_idx=%d\n",
    ...
    switch(att_idx)
    {
        ...
        {
            // Handle measurement interval value attribute

            // Allow writes to measurement interval when write support is enabled
            if (HTPT_IS_FEATURE_SUPPORTED(htpt_env->features, HTPT_MEAS_INTV_WR_SUP))
            {
                cfm->length = HTPT_MEAS_INTV_MAX_LEN;  // Allow 2-byte writes
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    // arch_printf("DEBUG: CUSTOM htpt_task.c gattc_write_req_ind_handler called: handle=0x%04X, conidx=%d, length=%d\n",
    //             param->handle, conidx, param->length);
    ...
    // Check attribute index and length
    ...
    else
    {
        switch(att_idx)
        {
            case HTS_IDX_MEAS_INTV_VAL:
            {
                ...
                // Validate measurement interval range
